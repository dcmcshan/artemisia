...
def main():
    ...
    assert len(matrix) == 302 and len(antiparasitic) == 77 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 480 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 64 and len(claims) == 108 and not matrix_missing
    assert bibliography_entries == 302
    result = {
        "validated": True,
