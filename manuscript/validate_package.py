...
def main():
    ...
    assert len(matrix) == 433 and len(antiparasitic) == 97 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 18 and len(manual) == 609 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 101 and len(claims) == 236 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 429
    result = {
        "validated": True,
