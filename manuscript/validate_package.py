...
def main():
    ...
    assert len(matrix) == 405 and len(antiparasitic) == 97 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 18 and len(manual) == 583 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 76 and len(claims) == 209 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 402
    result = {
        "validated": True,
