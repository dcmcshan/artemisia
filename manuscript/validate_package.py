...
def main():
    ...
    assert len(matrix) == 224 and len(antiparasitic) == 58 and len(interactions) == 15 and len(chemotypes) == 10 and len(safety) == 11 and len(manual) == 423 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 46 and not matrix_missing
    assert bibliography_entries == 240
    result = {
        "validated": True,
