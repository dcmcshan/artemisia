...
def main():
    ...
    assert len(matrix) == 493 and len(antiparasitic) == 97 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 21 and len(manual) == 669 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 141 and len(claims) == 297 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 489
    result = {
        "validated": True,
