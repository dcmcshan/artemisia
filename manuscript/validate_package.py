...
def main():
    ...
    assert len(matrix) == 135 and len(antiparasitic) == 28 and len(interactions) == 11 and len(chemotypes) == 10 and len(safety) == 4 and len(manual) == 334 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 37 and not matrix_missing
