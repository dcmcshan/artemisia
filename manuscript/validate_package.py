...
def main():
    ...
    assert len(matrix) == 532 and len(antiparasitic) == 117 and len(interactions) == 19 and len(chemotypes) == 10 and len(safety) == 21 and len(manual) == 758 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 177 and len(claims) == 337 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 528
    result = {
        "validated": True,
