...
def main():
    ...
    assert len(matrix) == 117 and len(antiparasitic) == 24 and len(interactions) == 11 and len(chemotypes) == 10 and len(safety) == 4 and len(manual) == 325 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 22 and not matrix_missing
    assert bibliography_entries == 133
    result = {
        "validated": True,
