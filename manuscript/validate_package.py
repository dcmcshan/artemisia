...
def main():
    ...
    assert len(matrix) == 260 and len(antiparasitic) == 63 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 452 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 42 and len(claims) == 75 and not matrix_missing
    assert bibliography_entries == 274
    result = {
        "validated": True,
