...
def main():
    ...
    assert len(matrix) == 308 and len(antiparasitic) == 78 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 14 and len(manual) == 486 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 64 and len(claims) == 114 and not matrix_missing
    assert bibliography_entries == 308
    result = {
        "validated": True,
