...
def main():
    ...
    assert len(matrix) == 243 and len(antiparasitic) == 61 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 11 and len(manual) == 432 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 57 and not matrix_missing
    assert bibliography_entries == 257
    result = {
        "validated": True,
