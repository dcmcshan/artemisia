...
def main():
    ...
    assert len(matrix) == 139 and len(antiparasitic) == 29 and len(interactions) == 11 and len(chemotypes) == 10 and len(safety) == 4 and len(manual) == 341 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 41 and not matrix_missing
    assert bibliography_entries == 154
    result = {
        "validated": True,
