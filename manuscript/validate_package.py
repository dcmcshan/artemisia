...
def main():
    ...
    assert len(matrix) == 248 and len(antiparasitic) == 62 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 436 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 62 and not matrix_missing
    assert bibliography_entries == 262
    result = {
        "validated": True,
