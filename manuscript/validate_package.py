...
def main():
    ...
    assert len(matrix) == 60 and len(antiparasitic) == 14 and len(interactions) == 11 and len(chemotypes) == 10 and len(safety) == 4 and len(manual) == 325 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 25 and len(claims) == 13 and not matrix_missing
    assert bibliography_entries == 76
    result = {
        "validated": True,
