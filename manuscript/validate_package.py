...
def main():
    ...
    assert len(appraisal) == len(antiparasitic) == 125
    assert len(full_text_queue) == len({row["source_id"] for row in antiparasitic}) == 106
    ...
    assert len(verification_ids) == len(set(verification_ids)) == 51
    ...
    assert len(matrix) == 540 and len(antiparasitic) == 125 and len(interactions) == 19 and len(chemotypes) == 10 and len(safety) == 21 and len(full_text_queue) == 106 and len(manual) == 1408 and len(manual_pmids) == len(manual) and len(manual_pmids & queue_pmids) == 1387 and len(supplementary) == 179 and len(claims) == 366 and not matrix_missing
