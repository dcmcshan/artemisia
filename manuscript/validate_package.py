...
def main():
    ...
    assert len(matrix) == 525 and len(antiparasitic) == 117 and len(interactions) == 19 and len(chemotypes) == 10 and len(safety) == 21 and len(manual) == 751 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 177 and len(claims) == 330 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 521
    result = {
        "validated": True,
