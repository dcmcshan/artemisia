...
def main():
    ...
    assert len(matrix) == 252 and len(antiparasitic) == 62 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 443 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 66 and not matrix_missing
    assert bibliography_entries == 266
    result = {
        "validated": True,
