...
def main():
    ...
    assert len(matrix) == 138 and len(antiparasitic) == 29 and len(interactions) == 11 and len(chemotypes) == 10 and len(safety) == 4 and len(manual) == 340 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 40 and not matrix_missing
    assert bibliography_entries == 153
    result = {
        "validated": True,
