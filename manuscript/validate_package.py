...
def main():
    ...
    assert len(matrix) == 286 and len(antiparasitic) == 74 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 467 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 50 and len(claims) == 97 and not matrix_missing
