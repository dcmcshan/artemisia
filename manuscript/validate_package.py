...
def main():
    ...
    assert len(matrix) == 380 and len(antiparasitic) == 97 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 17 and len(manual) == 559 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 64 and len(claims) == 185 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 377
    result = {
        "validated": True,
