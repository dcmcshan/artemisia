...
def main():
    ...
    assert len(appraisal) == len(antiparasitic) == 124
    assert len(full_text_queue) == len({row["source_id"] for row in antiparasitic}) == 105
    ...
    assert len(verification_ids) == len(set(verification_ids)) == 41
    ...
    assert len(matrix) == 539 and len(antiparasitic) == 124 and len(interactions) == 19 and len(chemotypes) == 10 and len(safety) == 21 and len(full_text_queue) == 105 and len(manual) == 1408 and len(manual_pmids) == len(manual) and len(manual_pmids & queue_pmids) == 1387 and len(supplementary) == 177 and len(claims) == 355 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 533
    result = {
        "validated": True,
