...
def main():
    ...
    assert len(matrix) == 476 and len(antiparasitic) == 97 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 21 and len(manual) == 652 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 129 and len(claims) == 280 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 472
    result = {
        "validated": True,
