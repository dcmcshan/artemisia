...
def main():
    ...
    assert len(matrix) == 522 and len(antiparasitic) == 114 and len(interactions) == 19 and len(chemotypes) == 10 and len(safety) == 21 and len(manual) == 748 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 177 and len(claims) == 327 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 518
    result = {
        "validated": True,
