...
def main():
    ...
    assert len(matrix) == 289 and len(antiparasitic) == 74 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 470 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 53 and len(claims) == 98 and not matrix_missing
    assert bibliography_entries == 288
    result = {
        "validated": True,
