...
def main():
    ...
    assert len(matrix) == 124 and len(antiparasitic) == 27 and len(interactions) == 11 and len(chemotypes) == 10 and len(safety) == 4 and len(manual) == 325 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 27 and not matrix_missing
    assert bibliography_entries == 140
    result = {
        "validated": True,
