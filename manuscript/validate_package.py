...
def main():
    ...
    assert len(matrix) == 246 and len(antiparasitic) == 62 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 13 and len(manual) == 435 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 60 and not matrix_missing
    assert bibliography_entries == 260
    result = {
        "validated": True,
