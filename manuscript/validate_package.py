...
def main():
    ...
    assert len(matrix) == 201 and len(antiparasitic) == 56 and len(interactions) == 13 and len(chemotypes) == 10 and len(safety) == 11 and len(manual) == 402 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 46 and not matrix_missing
    assert bibliography_entries == 217
    result = {
        "validated": True,
