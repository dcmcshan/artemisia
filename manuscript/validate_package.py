...
def main():
    ...
    assert len(matrix) == 528 and len(antiparasitic) == 117 and len(interactions) == 19 and len(chemotypes) == 10 and len(safety) == 21 and len(manual) == 754 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 177 and len(claims) == 333 and not matrix_missing
    assert bibliography_entries == len(set(re.findall(r"^@\w+\{([^,]+)", (ROOT / "references.bib").read_text(), re.MULTILINE))) == 524
    result = {
        "validated": True,
