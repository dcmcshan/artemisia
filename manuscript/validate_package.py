...
def main():
    ...
    assert len(matrix) == 240 and len(antiparasitic) == 61 and len(interactions) == 16 and len(chemotypes) == 10 and len(safety) == 11 and len(manual) == 429 and len({row['pmid'] for row in manual}) == len(manual) and len(supplementary) == 39 and len(claims) == 54 and not matrix_missing
    assert bibliography_entries == 254
    result = {
        "validated": True,
