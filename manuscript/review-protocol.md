...
The first completed verification tranche is archived in `full-text-verification.csv`. It covers 9 open-access source records and 12 linked evidence rows. “Full text verified” means that the accessible article was read and the listed preparation, analytical, parasite-model, outcome, and boundary fields were extracted; it does not mean that the study has passed the final inclusion or quantitative-harmonization gate. The remaining 95 queue sources retain the unverified status.
