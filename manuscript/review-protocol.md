...
The archived manual title/abstract decisions are stored in `manual-screening-decisions.csv`. They contain 482 high-priority decisions with bounded evidence scopes and rationale; 471 of those decisions correspond to records in the 1,387-record priority triage queue. The remaining 916 queue records are still pending manual review; the manually screened records include current-literature updates outside that archived queue. This log is not an exhaustive inclusion set.
