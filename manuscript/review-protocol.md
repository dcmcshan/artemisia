...
The archived manual title/abstract decisions are stored in `manual-screening-decisions.csv`. They contain 342 high-priority decisions with bounded evidence scopes and rationale. The remaining 1,051 records in the 1,387-record priority triage queue are still pending manual review; six manually screened records were added from current-literature updates outside that archived queue. This log is not an exhaustive inclusion set.
