...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 541-source manuscript with 556 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. The current render is 51 pages. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
- `artemisia-terpene-review-submission.pdf` — visually verified 29-page consolidated submission rendering with integrated author–year citations, a conventional reference list, three figures, and focused chemistry, malaria/wormwood, and comparative-genomics tables. Rebuild after citation conversion with `python build_submission_citations.py --in-place`, then `pandoc article-submission.md --from markdown --citeproc --bibliography=references.bib --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-submission.pdf`.
