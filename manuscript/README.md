...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 535-source manuscript with 540 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
...
- `full-text-verification.csv` — nineteen full-text extraction tranches: 52 sources (48 open-access and 4 publisher-full-text records) covering 61 evidence rows, with preparation-specific findings and explicit unresolved quantitative-eligibility gates.
