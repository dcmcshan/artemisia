...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 537-source manuscript with 544 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
...
- `evidence-matrix.csv` — 544-record specimen-, compound-, pathway-, genome-, review-, and assay-level evidence table.
...
- `full-text-verification.csv` — thirty-seven full-text extraction tranches: 77 sources covering 90 evidence rows, with preparation-specific findings and explicit unresolved quantitative-eligibility gates.
