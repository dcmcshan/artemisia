...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 542-source manuscript with 557 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. The current render is 51 pages. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
- `artemisia-terpene-review-submission.pdf` — visually verified 30-page consolidated submission rendering with integrated author–year citations, a conventional reference list, three figures, and focused chemistry, malaria/wormwood, and comparative-genomics tables. Rebuild after citation conversion with `python build_submission_citations.py --in-place`, then `pandoc article-submission.md --from markdown --citeproc --bibliography=references.bib --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='Times New Roman' -o artemisia-terpene-review-submission.pdf`.
- `artemisia-terpene-review-submission.docx` — visually verified 49-page Word submission artifact with the same cited manuscript, tables, figures, declarations, and reference list; table rows are configured not to split across pages. Use this source file for journal submission where Word is preferred.
...
- `evidence-matrix.csv` — 556-record specimen-, compound-, pathway-, genome-, review-, and assay-level evidence table.
...
- `antiparasitic-evidence.csv` — 133 quantitative and stage-aware parasite/vector assay records with host-control and translation boundaries.
...
- `screening-abstract-triage-summary.json` and `screening-abstract-triage.csv` — abstract-level ranking outputs whose internal status field remains `pending_manual_review` by design; the separate manual ledger covers all priority-queue records with bounded title/abstract decisions, and the validator reports that decision layer as complete while keeping full-text eligibility open.
- `manual-screening-decisions.csv` — 1,408-record title/abstract screening log with bounded decisions and rationale; all 1,387 priority-queue records are covered, plus 21 historical or out-of-queue records. Source-level verification is complete for the antiparasitic queue; final systematic-review inclusion is not claimed.
- `screening-exclusions.csv` and `build_screening_exclusions.py` — reproducible archive of the 16 explicitly excluded records, including correction/retraction audit status and recorded exclusion rationale.
...
- `full-text-eligibility-queue.csv` — source-linked full-text gate for all 114 unique antiparasitic source records (133 evidence rows), ranked for malaria, wormwood vermifuge, other parasite, and vector-control verification; all queued records now have source-level verification, while the separate quantitative audit documents which rows can only be reported descriptively.
- `full-text-verification.csv` — 114 source-level verification rows covering all 133 evidence rows, with preparation-specific findings and explicit unresolved chemistry, outcome, and translation limits; the latest records retain the *A. sieberi* seasonal-artemisinin/*Eimeria* study, associated *A. khorassanica* STL supplementary material, *A. indica* electronic-reprint mixture/host-control caveat, *A. campestris* inaccessible-tables, *A. scoparia* IC50-unit, *A. absinthium*/*H. contortus* indexed-text, two 2026 *A. annua* indexed-study, 2025 *A. cina* ewe-trial, low-dose *A. annua* coccidiosis caveats, the *A. herba-alba*/*H. gallinarum* poultry study, the *A. annua*/*H. meleagridis* in-vitro/in-vivo discordance control, the publisher-preview *A. cina*/*H. contortus* biomarker/gerbil study, the publisher-indexed *A. douglassiana* sesquiterpene-lactone/*T. cruzi* study, the open-access *L. mexicana* sesquiterpene-lactone redox-mechanism study, the indexed historical *A. annua* gelatin-capsule malaria study, the indexed 1987 artemisinin-combination malaria study, the related-species *A. brevifolia* sheep and *A. herba-alba* goat anthelmintic comparators, the *A. gilvescens*/*Anopheles anthropophagus* compound-comparison study, the thujone-rich *A. sieberi* poultry-mite study, the extraction-method *A. absinthium* acaricide study, the *A. annua*/*E. tenella* host-response study, the *A. inculta* schistosomiasis/marrow-safety study, the *A. maritima* sheep study, the *A. abyssinica*/*Leishmania* oil study, the four-species Ethiopian extract study, the purified-STL/*L. mexicana* study, the *A. annua*/*Culex* vector study, the *A. herba-alba* tick nanoemulsion, the *A. argyi* oil insecticidal study, and the indexed *A. ludoviciana* fasciolicide and *A. afra* mosquito-biting records.
