...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 541-source manuscript with 556 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
...
- `artemisia-terpene-review-submission.docx` — visually verified 44-page Word submission artifact with the same cited manuscript, tables, figures, declarations, and reference list; table rows are configured not to split across pages. Use this source file for journal submission where Word is preferred.
