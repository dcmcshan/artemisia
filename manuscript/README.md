...
- `artemisia-terpene-review-draft.pdf` — visually verified 44-page rendering of the current 533-source manuscript with 538 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
...
- `antiparasitic-evidence.csv` — 123 quantitative and stage-aware parasite/vector assay records with host-control and translation boundaries.
...
- `evidence-appraisal.csv` and `evidence-appraisal-protocol.md` — deterministic, provisional appraisal of all 123 antiparasitic records across chemistry resolution, phenotype context, host control, mechanism, and translation; not a meta-analytic quality score.
...
- `full-text-eligibility-queue.csv` — source-linked full-text gate for all 104 unique antiparasitic source records (123 evidence rows), ranked for malaria, wormwood vermifuge, other parasite, and vector-control verification.
- `full-text-verification.csv` — seven full-text extraction tranches: 38 open-access sources covering 46 evidence rows, with preparation-specific findings and explicit unresolved quantitative-eligibility gates.
