...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 434-source manuscript with 437 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
- `review-protocol.md` — inclusion, extraction, quality-assessment, and synthesis protocol.
- `evidence-matrix.csv` — 437-record specimen-, compound-, pathway-, genome-, review-, and assay-level evidence table.
...
- `supplementary-compound-specimen.csv` — 105 specimen-/tissue-/preparation-level compound records, including explicit non-terpene comparators.
...
- `manual-screening-decisions.csv` — 613-record manual title/abstract screening log with bounded decisions and rationale; the full queue remains open.
