...
- `artemisia-terpene-review-draft.pdf` — visually verified rendering of the current 514-source manuscript with 517 evidence records, including the focused malaria/wormwood evidence table, comparative-genomics table, vector-control evidence, structure-confirmed STL tranche, and regenerated figures. Rebuild with `pandoc article.md --from markdown --pdf-engine=xelatex --include-in-header=latex-header.tex --resource-path=. -V geometry:margin=0.5in -V mainfont='STIX Two Text' -o artemisia-terpene-review-draft.pdf`.
- `review-protocol.md` — inclusion, extraction, quality-assessment, and synthesis protocol.
- `evidence-matrix.csv` — 517-record specimen-, compound-, pathway-, genome-, review-, and assay-level evidence table.
...
- `antiparasitic-evidence.csv` — 109 quantitative, stage-aware parasite/vector assay records with host-control and translation boundaries.
...
- `manual-screening-decisions.csv` — 693-record manual title/abstract screening log with bounded decisions and rationale; the full queue remains open.
