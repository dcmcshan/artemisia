...
The package is deliberately a living scoping review rather than a completed systematic review. It contains 852 bounded title/abstract decisions, including 831 decisions within the priority queue and 21 historical or out-of-queue decisions; 556 of the 1,387 queue records remain pending and full-text eligibility is not complete. The PRISMA-ScR mapping is archived in `prisma-scr-checklist.md`; the search strings and retrieval counts are in `search-log.md` and `review-flow.md`. Reviews are used for discovery and context, while quantitative and mechanistic statements are preferentially anchored to primary studies.
