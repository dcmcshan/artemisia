...
Published comparative results already provide testable anchors: a recent *A. argyi* analysis reports 451 TPS genes split between two haplotypes (221 and 230), while the chromosome-scale study links WGD and duplication modes to TPS expansion and identifies a germacrene D synthase cluster. The latter study also reports pathway elucidation for germacrenes, (+)-borneol, and (+)-camphor. These observations are catalogued in the KB’s `gene-family-evidence.csv`; they are not treated as a substitute for a common-annotation, orthology-aware rerun.
